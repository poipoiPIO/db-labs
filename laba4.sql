...
CREATE table if not EXISTS Студент (
  ...
  PRIMARY KEY (нс),
  FOREIGN KEY (нг) REFERENCES Группа(нг)
);
...
CREATE TABLE if not EXISTS Преподаватель (
  ...
  FOREIGN KEY (ккаф) REFERENCES Кафедра(ккаф)
);
...
CREATE table if not EXISTS Изучение (
  ...
  FOREIGN KEY (кп) REFERENCES Предмет(кп),
  FOREIGN KEY (табн) REFERENCES Преподаватель(табн),
  FOREIGN KEY (нг) REFERENCES Группа(нг)
);
...
CREATE table if not EXISTS Успеваемость (
  ...
  кп     VARCHAR(2) NOT NULL, -- код предмета
  табн   VARCHAR(4) NOT NULL, -- таб. н. преподователя
  видз   VARCHAR(3) NOT NULL, -- вид занятий
  оценка INTEGER NULL,
  
  FOREIGN KEY (нг, нс) REFERENCES Студент(нг, нс),
  FOREIGN KEY (нг, кп, табн, видз) REFERENCES Изучение(нг, кп, табн, видз)
);
